...
Deno.serve(async (req) => {
  ...
  const { query, municipalityId, matchCount = 4 } = body;
  ...
  try {
    ...
    const { data: chunks, error: rpcErr } = await supabase.rpc("match_organ_memory", {
    ...
    const context = chunks && chunks.length > 0
      ? `MEMÓRIA DO ÓRGÃO (documentos anteriores deste órgão):\n${chunks.map((c: any, i: number) => `[${i + 1}]${c.document_tipo ? ` (${String(c.document_tipo).toUpperCase()})` : ""} ${c.content}`).join("\n\n")}`
      : "";
