      ...
      {open && (
        ...
        >
          ...
          <div className="flex flex-col gap-2 pt-2 border-t border-white/[0.06]">
            ...
            <Link
              to="/signup"
              className="text-sm font-medium text-center bg-cyan-400 text-[#080D14] px-5 py-2.5 rounded-lg"
            >
              Começar grátis
            </Link>
          </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center">
        ...
        >
          ...
          <span
            ...
            style={{
              backgroundImage: "linear-gradient(135deg, hsl(190 95% 50%) 0%, hsl(265 90% 60%) 50%, hsl(190 95% 50%) 100%)",
            }}
          >
            completo
          </span>
        ...
        >
          <Link
            to="/signup"
            className="group flex items-center gap-2 px-8 py-3.5 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-[#080D14] font-semibold text-sm transition-all duration-200 shadow-[0_0_30px_-6px_hsl(190_95%_50%/0.6)] hover:shadow-[0_0_40px_-4px_hsl(190_95%_50%/0.7)]"
          >
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex -space-x-1">
              {["C", "M", "A", "R", "L"].map((l, i) => (
                <div
                  key={i}
                  className="w-6 h-6 rounded-full border border-[#0B0E14] bg-gradient-to-br from-cyan-500/40 to-violet-500/40 flex items-center justify-center text-[9px] text-white font-medium"
                >
                  {l}
                </div>
              ))}
        ...
        <AnimatedSection className="text-center mb-20">
          ...
          >
            ...
            <span className="bg-clip-text text-transparent"
              style={{ backgroundImage: "linear-gradient(135deg, hsl(190 95% 50%), hsl(265 90% 60%))" }}>
              ecossistema
            </span>
          </h2>
        </AnimatedSection>
          ...
          {VERTICALS.map((v, i) => {
            ...
            return (
              <AnimatedSection key={v.id} delay={i * 0.1}>
                <div
                  className={cn(
                    "relative group rounded-2xl border p-8 transition-all duration-300 hover:-translate-y-1",
                    s.border, s.bg,
                    "hover:" + s.glow
                  )}
                >
                  {/* Top accent */}
                  <div className={cn("absolute top-0 left-8 right-8 h-px", `bg-gradient-to-r ${s.gradient} opacity-60`)} />

                  <div className={cn("inline-flex px-3 py-1 rounded-full text-xs font-medium mb-6", s.badgeBg)}>
                    {v.badge}
                  </div>

                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-4", s.bg, "border", s.border)}>
                  ...
                  <ul className="space-y-3 mb-8">
                    ...
                    {v.features.length > 4 && (
                      <li className="text-xs text-white/30">
                        +{v.features.length - 4} funcionalidades
                      </li>
                    )}
                  </ul>
    ...
    <section id={vertical.id} className="py-28 px-6">
      <div className="max-w-7xl mx-auto">
        <div className={cn("grid md:grid-cols-2 gap-16 items-center", isReversed && "md:[&>*:first-child]:order-2")}>
          {/* Content */}
          <AnimatedSection>
            <div className={cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium mb-6", s.badgeBg)}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {vertical.features.map((f, i) => (
                <motion.div
                  ...
                  className={cn(
                    "rounded-xl border p-5 transition-all duration-200 group hover:border-opacity-50",
                    s.border, "bg-white/[0.02] hover:bg-white/[0.04]"
                  )}
                >
                  <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center mb-3", s.bg, "border", s.border)}>
    ...
    <section className="py-24 px-6">
      <div className="max-w-7xl mx-auto">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(ellipse 80% 60% at 50% 100%, hsl(190 95% 50% / 0.05), transparent)",
          }}
        >
          <AnimatedSection className="text-center mb-16">
            <p className="text-xs text-white/30 tracking-widest uppercase font-medium mb-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
            {STATS.map((s, i) => (
              <AnimatedSection key={s.label} delay={i * 0.1} className="text-center">
                ...
                >
                  {s.suffix === "B+" ? (
                    <>R$<AnimatedCounter target={s.value} />{s.suffix}</>
                  ) : (
...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      "Geradores ilimitados (ETP, TR)",
      "Notebook IA completo",
      "Validador de minutas",
      "Cotação de preços IA",
...
function PricingSection() {
  return (
    <section id="preços" className="py-28 px-6">
      <div className="max-w-7xl mx-auto">
        <AnimatedSection className="text-center mb-20">
          <p className="text-xs text-white/30 tracking-widest uppercase font-medium mb-4">
            Planos e preços
          </p>
          <h2
            ...
            <AnimatedSection key={plan.name} delay={i * 0.1}>
              <div
                className={cn(
                  ...
                  plan.highlight
                    ? "border-cyan-400/30 bg-cyan-400/5 shadow-[0_0_60px_-12px_hsl(190_95%_50%/0.3)]"
              ...
              >
                {plan.highlight && (
                  <>
                    ...
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-cyan-400 text-[#080D14] text-xs font-semibold">
                      Mais popular
                    </div>
                  </>
                )}
                ...
                <a
                  ...
                  className={cn(
                    ...
                    plan.highlight
                      ? "bg-cyan-400 hover:bg-cyan-300 text-[#080D14] shadow-[0_0_20px_-4px_hsl(190_95%_50%/0.5)]"
      ...
      <div className="max-w-4xl mx-auto">
        <AnimatedSection>
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse 70% 80% at 50% 50%, hsl(190 95% 50% / 0.08) 0%, hsl(265 90% 60% / 0.04) 50%, transparent 100%), hsl(222 24% 8%)",
            }}
          >
            ...
            <p className="text-lg text-white/40 max-w-xl mx-auto mb-12 leading-relaxed">
              Junte-se a milhares de gestores que já profissionalizaram suas compras públicas
              com o Intelicite.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                to="/signup"
                className="group flex items-center gap-2 px-8 py-4 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-[#080D14] font-semibold transition-all duration-200 shadow-[0_0_40px_-8px_hsl(190_95%_50%/0.6)]"
              >
              ...
              <a
                ...
                className="flex items-center gap-2 px-8 py-4 rounded-xl border border-white/10 text-white/60 hover:text-white hover:border-white/20 font-medium transition-all duration-200"
              >
            ...
            <p className="mt-8 text-xs text-white/20">
              Sem necessidade de cartão de crédito · Cancele quando quiser · Dados 100% seguros
            </p>
          </div>
        </AnimatedSection>
      </div>
        ...
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="md:col-span-1">
            ...
            <p className="text-sm text-white/35 leading-relaxed">
              O ecossistema completo de inteligência para licitações públicas do Brasil.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                { label: "Intelicite — Gestores", href: "/signup" },
                { label: "Licitante — Empresas", href: "#licitante" },
                { label: "Consultor — Especialistas", href: "#consultor" },
              ].map((l) => (
                <li key={l.label}>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                ...
                { label: "Contato", href: "mailto:contato@intelicite.com.br" },
              ].map((l) => (
                <li key={l.label}>
                  <a href={l.href} className="text-sm text-white/35 hover:text-white transition-colors">
                    {l.label}
